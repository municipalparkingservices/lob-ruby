  ...
  class SortByDateModified
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lob::SortByDateModified` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lob::SortByDateModified`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      date_modified_validator = EnumAttributeValidator.new('String', ["asc", "desc"])
      return false unless date_modified_validator.valid?(@date_modified)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] date_modified Object to be assigned
    def date_modified=(date_modified)
      validator = EnumAttributeValidator.new('String', ["asc", "desc"])
      unless validator.valid?(date_modified)
        fail ArgumentError, "invalid value for \"date_modified\", must be one of #{validator.allowable_values}."
      end
      @date_modified = date_modified
    end
    ...
    def hash
      [date_created, date_modified].hash
    end
