  ...
  class Zip
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lob::Zip` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lob::Zip`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^us_zip_[a-zA-Z0-9]+$/)
      if @id !~ pattern
    ...
    def valid?
      return false if !@zip_code.nil? && @zip_code !~ Regexp.new(/^\d{5}$/)
      return false if @id.nil?
      return false if @id !~ Regexp.new(/^us_zip_[a-zA-Z0-9]+$/)
      return false if @cities.nil?
      return false if @zip_code_type.nil?
      return false if @object.nil?
      object_validator = EnumAttributeValidator.new('String', ["us_zip_lookup"])
    ...
    def object=(object)
      validator = EnumAttributeValidator.new('String', ["us_zip_lookup"])
    ...
    def hash
      [zip_code, id, cities, zip_code_type, object].hash
    end
