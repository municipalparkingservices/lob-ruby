  ...
  class TemplateUpdate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lob::TemplateUpdate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lob::TemplateUpdate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@published_version.nil? && @published_version !~ pattern
        invalid_properties.push("invalid value for \"published_version\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if !@published_version.nil? && @published_version !~ Regexp.new(/^vrsn_[a-zA-Z0-9]+$/)
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] published_version Value to be assigned
    def published_version=(published_version)
      pattern = Regexp.new(/^vrsn_[a-zA-Z0-9]+$/)
      if !published_version.nil? && published_version !~ pattern
        fail ArgumentError, "invalid value for \"published_version\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [description, published_version].hash
    end
