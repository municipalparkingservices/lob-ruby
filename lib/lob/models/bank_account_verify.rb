  ...
  class BankAccountVerify
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lob::BankAccountVerify` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lob::BankAccountVerify`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] amounts Value to be assigned
    def amounts=(amounts)
      if amounts.nil?
        fail ArgumentError, 'amounts cannot be nil'
      end

      if amounts.length > 2
        fail ArgumentError, 'invalid value for "amounts", number of items must be less than or equal to 2.'
      end

      if amounts.length < 2
        fail ArgumentError, 'invalid value for "amounts", number of items must be greater than or equal to 2.'
      end
