...
module Lob
  class UsAutocompletions
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^us_auto_[a-zA-Z0-9]+$/)
      if !@id.nil? && @id !~ pattern
        invalid_properties.push("invalid value for \"id\", must conform to the pattern #{pattern}.")
      end

      if !@suggestions.nil? && @suggestions.length > 10
        invalid_properties.push('invalid value for "suggestions", number of items must be less than or equal to 10.')
      end

      if !@suggestions.nil? && @suggestions.length < 0
        invalid_properties.push('invalid value for "suggestions", number of items must be greater than or equal to 0.')
      end
    ...
    def valid?
      return false if !@id.nil? && @id !~ Regexp.new(/^us_auto_[a-zA-Z0-9]+$/)
      return false if !@suggestions.nil? && @suggestions.length > 10
      return false if !@suggestions.nil? && @suggestions.length < 0
      object_validator = EnumAttributeValidator.new('String', ["us_autocompletion"])
    ...
    def suggestions=(suggestions)
      if !suggestions.nil? && suggestions.length > 10
        fail ArgumentError, 'invalid value for "suggestions", number of items must be less than or equal to 10.'
      end

      if !suggestions.nil? && suggestions.length < 0
        fail ArgumentError, 'invalid value for "suggestions", number of items must be greater than or equal to 0.'
      end
    ...
    def object=(object)
      validator = EnumAttributeValidator.new('String', ["us_autocompletion"])
